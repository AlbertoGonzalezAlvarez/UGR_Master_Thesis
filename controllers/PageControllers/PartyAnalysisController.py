...
def build_topic_chart():
	...
	figure = go.Figure(data=[
		go.Bar(
			...
			showlegend=False if WebApplication.is_mobile_device else True,
			hovertemplate='Porcentaje de tiempo: %{y:.2f}<br>'
		) for party_id in PARTY_CONFIG],
		layout=dict(
			barmode='group',
			margin=dict(l=0, r=40, t=40),
			xaxis_title='' if WebApplication.is_mobile_device else 'Temas',
			yaxis_title='' if WebApplication.is_mobile_device else 'Porcentaje de tiempo',
			**PLOT_BASE_CONFIG
		)
	)
...
def build_topic_chart_duration():
	...
	figure = go.Figure(go.Bar(
		...
		layout=dict(
			...
			margin=dict(l=0, r=40, t=40),
			xaxis_title='' if WebApplication.is_mobile_device else 'Temas',
			yaxis_title='' if WebApplication.is_mobile_device else 'Minutos estimados',
			**PLOT_BASE_CONFIG
		)
	)
	return figure
